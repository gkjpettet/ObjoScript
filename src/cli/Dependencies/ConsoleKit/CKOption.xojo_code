#tag Class
Protected Class CKOption
	#tag Method, Flags = &h0
		Sub AddAllowedValue(ParamArray values() As String)
		  If AllowedValues.LastIndex <> -1 Then
		    Raise new CKOptionException("You cannot set both allowed values and disallowed values")
		  End If
	...
	#tag Method, Flags = &h0
		Sub AddDisallowedValue(ParamArray values() As String)
		  If AllowedValues.LastIndex <> -1 Then
		    Raise New CKOptionException("You cannot set both disallowed values and allowed values")
		  End If
	...
	#tag Method, Flags = &h0
		Sub Constructor(shortKey As String, longKey As String, description As String, type As OptionTypes = OptionTypes.String)
		  //
		  ...
		  If shortKey = "" and longKey = "" Then
		    Raise New CKOptionException("Option must specify at least one key.")
		  End If
		  If shortKey.Length > 1 Then
		    Raise New CKOptionException("Short Key is optional but may only be one character: " + shortKey)
		  End If
		  If longKey.Length = 1 Then
		    Raise New CKOptionException("Long Key is optional but must be more than one character: " + longKey)
		  End If
	...
	#tag Method, Flags = &h0
		Sub HandleValue(value As String)
		  ...
		  If AllowedValues.LastIndex <> -1 And AllowedValues.IndexOf(value) = -1 Then
		    Raise New CKOptionException("The key value is not allowed: " + value)
		  End If
		  If DisallowedValues.LastIndex <> -1 And DisallowedValues.IndexOf(value) <> -1 Then
		    Raise New CKOptionException("The key value is dissallowed: " + value)
		  End If
		  ...
		  Case OptionTypes.Directory
		    If value <> "" Then
		      newValue = CKOptionParser.GetRelativeFolderItem(value)
		    End If
