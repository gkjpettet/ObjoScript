...
Inherits DesktopTreeView
	#tag Method, Flags = &h0
		Sub Display(vm As ObjoScript.VM, frame As ObjoScript.CallFrame)
		  ...
		  // Handle `this`
		  Var this As Variant = vm.GetFrameSlotValue(frame, 0)
		  If this IsA ObjoScript.Instance Or this IsA ObjoScript.Klass Then
		    Me.AppendNode(VariableToNode("this", this))
		  End If
		  
		  // Optional arguments.
		  For i As Integer = 0 To frame.Function_.Parameters.LastIndex
		    Var slot As Integer = i + 1 // Add one because slot 0 is the function but the parameters array is 0-based.
		    Me.AppendNode(VariableToNode(frame.Function_.Parameters(i), vm.GetFrameSlotValue(frame, slot)))
		  Next i
		  
		  // Locally declared variables.
		  For Each entry As DictionaryEntry In frame.Locals
		    Me.AppendNode(VariableToNode(entry.Key, vm.GetFrameSlotValue(frame, entry.Value)))
		  Next entry
